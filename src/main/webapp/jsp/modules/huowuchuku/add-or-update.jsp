...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑货物出库</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>货物出库管理</span></li>
								<li class="breadcrumb-item active"><span>编辑货物出库</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">货物出库信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>司机姓名</label>
										<select  class="form-control"  name="sijixingming" id="sijixingmingSelect" onchange="sijixingmingSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>司机手机</label>
										<input id="sijishouji" name="sijishouji" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>身份证</label>
									...
									<div class="form-group col-md-6">
										<label>门店编号</label>
										<select  class="form-control"  name="mendianbianhao" id="mendianbianhaoSelect" onchange="mendianbianhaoSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>门店名称</label>
										<input id="mendianmingcheng" name="mendianmingcheng" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>门店地址</label>
										<input id="mendiandizhi" name="mendiandizhi" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>咨询电话</label>
										<input id="zixundianhua" name="zixundianhua" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>出库时间</label>
									...
									<div class="form-group col-md-6">
										<label>出库备注</label>
										<input name="chukubeizhu" id="chukubeizhu" name="chukubeizhu" class="form-control" placeholder="出库备注">
									</div>
	...
	<script>
		...
		var ruleForm = {};
		var crossRuleForm = {};
				var huowushuliangKucun = 0;


		function sijixingmingSelect2(){
			http("option/sijixinxi/sijixingming","GET",{
				tableName: "sijixinxi",
				columnName: "sijixingming"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
					$("#sijixingmingSelect").append(option)
					for(var i=0;i<options.length;i++){
						var sijixingmingOption = document.createElement("option");
						sijixingmingOption.setAttribute("value",options[i]);
						sijixingmingOption.setAttribute("class","sijixingmingOption");
						if(ruleForm.sijixingming == options[i]){
							sijixingmingOption.setAttribute("selected","selected");
						}
						sijixingmingOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.sijixingming != null && ruleForm.sijixingming != '' && options[i] == ruleForm.sijixingming){
							sijixingmingOption.setAttribute("selected","selected");
						}
						$("#sijixingmingSelect").append(sijixingmingOption);
					}
				}
			});
		}		
		function sijixingmingSelect2Change(){
			var options = document.getElementById("sijixingmingSelect").options;
			var index = document.getElementById("sijixingmingSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/sijixinxi/sijixingming","GET",{
				tableName: "sijixinxi",
				columnName: "sijixingming",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					if(res.data != null && res.data.sijishouji != null){
						$("#sijishouji").val(res.data.sijishouji);
					}
					if(res.data != null && res.data.shenfenzheng != null){
						$("#shenfenzheng").val(res.data.shenfenzheng);
					}
					if(res.data != null && res.data.mendianmingcheng != null){
						$("#mendianmingcheng").val(res.data.mendianmingcheng);
					}
					if(res.data != null && res.data.mendiandizhi != null){
						$("#mendiandizhi").val(res.data.mendiandizhi);
					}
					if(res.data != null && res.data.zixundianhua != null){
		...
		function mendianbianhaoSelect2(){
			http("option/mendianxinxi/mendianbianhao","GET",{
				tableName: "mendianxinxi",
				columnName: "mendianbianhao"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
					$("#mendianbianhaoSelect").append(option)
					for(var i=0;i<options.length;i++){
						var mendianbianhaoOption = document.createElement("option");
						mendianbianhaoOption.setAttribute("value",options[i]);
						mendianbianhaoOption.setAttribute("class","mendianbianhaoOption");
						if(ruleForm.mendianbianhao == options[i]){
							mendianbianhaoOption.setAttribute("selected","selected");
						}
						mendianbianhaoOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.mendianbianhao != null && ruleForm.mendianbianhao != '' && options[i] == ruleForm.mendianbianhao){
							mendianbianhaoOption.setAttribute("selected","selected");
						}
						$("#mendianbianhaoSelect").append(mendianbianhaoOption);
					}
				}
			});
		}		
		function mendianbianhaoSelect2Change(){
			var options = document.getElementById("mendianbianhaoSelect").options;
			var index = document.getElementById("mendianbianhaoSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/mendianxinxi/mendianbianhao","GET",{
				tableName: "mendianxinxi",
				columnName: "mendianbianhao",
				columnValue: colVal
			},(res)=>{
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "huowuchuku/info/" + id,
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
				if(crossFlag) {
					if(Number($('#huowushuliang').val())<1) {
						alert('请输入正确的货物数量');
						return false;
					}
					if(Number(huowushuliangKucun) < Number($('#huowushuliang').val())){
						alert('货物数量不能大于'+huowushuliangKucun+'！请重新输入');
						return false;
					}
				}
				let data = {};
				...
				if(crossrefid && crossuserid) {
					...
					http("huowuchuku/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("huowuchuku/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("huowuchuku/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#cangkuweizhi").val(ruleForm.cangkuweizhi);	
						...
						$("#sijishouji").val(ruleForm.sijishouji);	
			...
			$("#chukubeizhu").val(ruleForm.chukubeizhu);	
		}		
		//图片显示
		function showImg(){
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.huowushuliang != null && res.data.huowushuliang != ''){
						...
						huowushuliangKucun = res.data.huowushuliang;
					...
					if(res.data != null && res.data != '' && res.data.sijixingming != null && res.data.sijixingming != ''){

					}
					if(res.data != null && res.data != '' && res.data.sijishouji != null && res.data.sijishouji != ''){

						$("#sijishouji").val(res.data.sijishouji);
						$("#sijishouji").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.chukubeizhu != null && res.data.chukubeizhu != ''){

						$("#chukubeizhu").val(res.data.chukubeizhu);
						$("#chukubeizhu").attr('readonly','readonly');
					}
				}
			});  
            }
