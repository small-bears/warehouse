...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑货物信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>编辑货物信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">货物信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>有效期至</label>
										<div id="youxiaoqizhiDate" class="input-append date form_datetime">
											<input id="youxiaoqizhi-input" name="youxiaoqizhi" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										<label>仓库编号</label>
										<select  class="form-control"  name="cangkubianhao" id="cangkubianhaoSelect" onchange="cangkubianhaoSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>仓库名称</label>
										<input id="cangkumingcheng" name="cangkumingcheng" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>仓库位置</label>
										<input id="cangkuweizhi" name="cangkuweizhi" class="form-control"  value="" readonly>
									</div>
	...
	<script>
		...
		function cangkubianhaoSelect2(){
			http("option/cangkuxinxi/cangkubianhao","GET",{
				tableName: "cangkuxinxi",
				columnName: "cangkubianhao"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
					$("#cangkubianhaoSelect").append(option)
					for(var i=0;i<options.length;i++){
						var cangkubianhaoOption = document.createElement("option");
						cangkubianhaoOption.setAttribute("value",options[i]);
						cangkubianhaoOption.setAttribute("class","cangkubianhaoOption");
						if(ruleForm.cangkubianhao == options[i]){
							cangkubianhaoOption.setAttribute("selected","selected");
						}
						cangkubianhaoOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.cangkubianhao != null && ruleForm.cangkubianhao != '' && options[i] == ruleForm.cangkubianhao){
							cangkubianhaoOption.setAttribute("selected","selected");
						}
						$("#cangkubianhaoSelect").append(cangkubianhaoOption);
					}
				}
			});
		}		
		function cangkubianhaoSelect2Change(){
			var options = document.getElementById("cangkubianhaoSelect").options;
			var index = document.getElementById("cangkubianhaoSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/cangkuxinxi/cangkubianhao","GET",{
				tableName: "cangkuxinxi",
				columnName: "cangkubianhao",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					if(res.data != null && res.data.cangkumingcheng != null){
						$("#cangkumingcheng").val(res.data.cangkumingcheng);
					}
					if(res.data != null && res.data.cangkuweizhi != null){
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "huowuxinxi/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("huowuxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("huowuxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("huowuxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#youxiaoqizhiDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#youxiaoqizhi-input").val(ruleForm.youxiaoqizhi);
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.huowushuliang != null && res.data.huowushuliang != ''){
						...
						$("#huowushuliang").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.youxiaoqizhi != null && res.data.youxiaoqizhi != ''){

						$("#youxiaoqizhi-input").val(res.data.youxiaoqizhi);
						$('#youxiaoqizhiFileName').val(res.data.youxiaoqizhi)
					}
